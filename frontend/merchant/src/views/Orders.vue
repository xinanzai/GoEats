    ...
    >
      <div v-if="currentOrder">
        <el-descriptions :column="2" border class="order-info">
          ...
          <el-descriptions-item
            label="取消原因"
            v-if="currentOrder.cancel_reason"
            :span="2"
          >
...
import { getMyOrders, prepareOrder, deliverOrder, completeOrder } from '@/api/orders'
...
async function handlePrepare(row) {
  try {
    await ElMessageBox.confirm('确定开始制作该订单吗？', '提示', {
...
async function handleDeliver(row) {
  try {
    await ElMessageBox.confirm('确定开始配送该订单吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
